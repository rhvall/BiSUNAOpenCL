...
//        ModuleDebug::printGraph(this, "problem.dot");
    ...
    if (sum < -MAX_NEURON_OUTPUT || sum > MAX_NEURON_OUTPUT) {
        ...
        //printf("neuron_state!! nan, id %d sum %f \n",id, sum);
    }
#else
...
#ifdef CONTINUOUS_PARAM
        printf("neuron_state!! nan, id %d sum %f neuron output %f\n", id, sum, neuron_output);
#else
        printf("neuron_state!! nan, id %d sum %i neuron output %i\n", id, sum, neuron_output);
#endif
	}
    ...
    ParameterType in = previous_neuron_state[neuron_index];
    ParameterType beta = betaFromFR(fr);
	
#ifdef CONTINUOUS_PARAM
    //use Windrow Hoff for slower neurons
    //printf("before beta %f, neuron_state %f\n", beta, neuron_state[neuron_index]);
    neuron_state[neuron_index] = in + beta * (neuron_output - in);
#else
    neuron_state[neuron_index] = (in ^ (beta & xnor(neuron_output, in)));
#endif

	if (isnan(neuron_state[neuron_index]) || isinf(neuron_state[neuron_index])) {
#ifdef CONTINUOUS_PARAM
        printf("neuron_state!! nan, id %d beta %f neuron output %f \n", id, beta, neuron_output);
#else
        printf("neuron_state!! nan, id %d beta %i neuron output %i \n", id, fr, neuron_output);
#endif
	}

	//printf("beta %f, neuron_state %f\n",beta, neuron_state[neuron_index]);
